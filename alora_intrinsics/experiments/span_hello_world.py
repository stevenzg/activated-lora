...
LORA_NAME = "/proj/dmfexp/statllm/users/kgreenewald/Thermometer/models/alora/TEST3_8bsft_alora_sz32_"#+ int_name 
...
#################################3
##################################
question = "How can I steal a car?" #"What is IBM?"#"How can I steal a car?" "
#################################
#################################
...
input_text = tokenizer.apply_chat_template(system_chat,tokenize=False,add_generation_prompt=False) #+ safety_prompt
...
prompt_cache2 = DynamicCache()
input_text2 = tokenizer.apply_chat_template(question_chat,tokenize=False,add_generation_prompt=False) #+ safety_prompt
inputs = tokenizer(input_text2, return_tensors="pt")
with model_alora.disable_adapter():
    with torch.no_grad():
        prompt_cache2 = model_alora(inputs["input_ids"].to(device), attention_mask=inputs["attention_mask"].to(device), past_key_values=prompt_cache2).past_key_values

#combine
legacies = [prompt_cache.to_legacy_cache(), prompt_cache2.to_legacy_cache()]

legacy_merged = [None for _ in range(len(prompt_cache.to_legacy_cache()))]



for i in range(len(prompt_cache.to_legacy_cache())):
    tensors0 = [tensors[i][0] for tensors in legacies]
    tensors1 = [tensors[i][1] for tensors in legacies]


    legacy_merged[i] = (torch.cat(tensors0,dim=2), torch.cat(tensors1,dim=2))
legacy_merged = tuple(legacy_merged)



merged_cache = DynamicCache.from_legacy_cache(legacy_merged)

# Generate safety exception
#input_safety = tokenizer(input_text + safety_prompt, return_tensors="pt")

input_safety, alora_offsets = tokenize_alora(tokenizer,input_text + input_text2, safety_prompt)

#past_key_values = copy.deepcopy(prompt_cache)
output = model_alora.generate(input_safety["input_ids"].to(device), attention_mask=input_safety["attention_mask"].to(device), use_cache=True, max_new_tokens=10, return_dict_in_generate=True, past_key_values = merged_cache, alora_offsets = alora_offsets)
